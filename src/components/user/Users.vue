...
export default {
  ...
  methods: {
    ...
    getUserList () {
      this.$api.users().then((res) => {
        if (res.data.status === 200) {
          ...
          this.value = res.data.result[0].mg_state
        }
      })
    },
    // 监听pagesize改变的事件
    ...
    saveRoleInfo () {
      ...
      this.$api.UserRoleById({
        id: this.selectedRoleId
      }).then((res) => {
        if (res.data.status !== 200) {
          return this.$message.error('用户角色查找失败')
        } else {
          this.$api.updateUser({
            id: this.userInfo.id,
